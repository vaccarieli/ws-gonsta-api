...
export const downloadMedia = async (m: any, webhookData: any, mediaTypes: any) => {
    const messages = m?.messages?.[0];

    if (messages?.message) {
        const mediaType = Object.keys(messages?.message).find((type) => type in mediaTypes) ?? "";

        if (Object.keys(mediaTypes).includes(mediaType)) {
            let buffer = Buffer.from([]);
            try {
                const stream = await downloadContentFromMessage(messages?.message?.[mediaType], mediaTypes[mediaType]);
