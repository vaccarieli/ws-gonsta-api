...
export const Paths = {
    ...
    user_data_path: function (user: string) {
        return `${this.cwd}/data/${user.toUpperCase()} ${getMacAddress()?.toString().toUpperCase()}/`;
    },
    ...
    user_data_file: function (user: string) {
        if (!fs.existsSync(this.user_data_path(user))) fs.mkdirSync(this.user_data_path(user), {recursive: true});
...
export const wasSentRecently = (webHookData: any, obj: any, user: any) => {
    if (obj) {
        for (let innerIndex = 0; innerIndex < obj.length; innerIndex++) {
            if (webHookData["fromMe"] || (obj[innerIndex]["fromMe"] && obj[innerIndex]["userInstance"] === user)) {
                return true;
            }
        }
    }
